...
mob/squad_verbs
	verb
		Leave_Squad()
			if(squad && input2(src,"Are you sure you want to leave your squad?","Leave Squad",list("Yes","No")) == "Yes" && squad)
				...
				if(squad.leader == name)
					spawn()
						SendInterserverMessage("squad_delete", list("squad" = squad.name))
					del squad
				if(squad)
	...
	leader
		verb
			Invite_to_Squad(mob/M in oview())
				if(M.faction && faction && M.faction.village == faction.village && squad && !M.squad)
					...
					if((faction in list(leaf_faction,mist_faction,sand_faction)) && name == faction.leader)
						max_squad = 10
					var/squad_members = SendInterserverMessage("squad_member_count", list("squad" = squad.name))
					...
					if((squad_members+squad.pending_invites) < max_squad)
						...
						if(accepted == "Yes" && squad)
							squad_members = SendInterserverMessage("squad_member_count", list("squad" = squad.name))
							squad_members = max(squad_members, squad.online_members.len)
