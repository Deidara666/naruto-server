obj/mapinfo
	...
	PlayerLeft(mob/human/M)
		if(!istype(M, /mob/human/player/npc) && !istype(M, /mob/human/player/npc/bunshin) && !istype(M, /mob/human/player/npc/kage_bunshin) && M.faction)
			...
			//online_admins << "[oX],[oY] ([z]) [village_control]: PlayerLeft([M]) [M.faction] [M.faction.village] [village_players[M.faction.village]]"
			RefreshAlert()
			if(in_war)
...
proc
	Setup_Map()
		//locate(92,9,2) [Center of map]
		var/list/map_turfs = block(locate(84,1,2), locate(100,17,2))
		if(fexists("war.sav"))
			...
			for(var/z in 1 to world.maxz)
				...
				var/turf/T = locate(MAP_START_X+Minfo.oX-1, MAP_START_Y-Minfo.oY, 2)
				map_turfs -= T
			...
			for(var/z in 1 to world.maxz)
				...
				if(Minfo.oY <= 2)
					...
					if(Minfo.oY >= 6 && Minfo.oY <= 8 && Minfo.oX <= 2)
						...
						if(Minfo.oY >= 7 && Minfo.oY <= 8 && Minfo.oX <= 1)
							Minfo.can_capture = 0
							Minfo.base_capture = 0
					else
						if(Minfo.oX >= 8)
