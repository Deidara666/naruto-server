					...
					if(length(msg) <= 500&&usr.say==1)
						...
						spawn()
							if(x && x.client && winget(x, "whisper__[usr.realname]", "is-visible") != "true")
								if(x.ckey in admins)
									x << "You have recieved a whisper message from <a href='?src=\ref[usr];action=mute' class='admin_link'>[usr.realname]</a>. <a href='?[list2params(list("src"="\ref[x]", "action"="view_whisper", "name"="[usr.realname]"))]'>\[View]</a>"
								else
						...
						if(x.ckey in admins)
							x << output("<a href='?src=\ref[usr];action=mute' class='admin_link'>[usr]</a>: <font color=#52ad4d>[html_encode(msg)]", "whisper__[usr.realname].whisper_chat_output")
						else
