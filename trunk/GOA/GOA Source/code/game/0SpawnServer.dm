...
var
	address_of_other_server = "byond://mmservices.homelinux.org:1117"
	maxplayers = 100
	block_alts = 0
...
world/New()
	...
	lp_mult = Dict["LP_Multiplier"]
	address_of_other_server=Dict["Save_Address"]

	if(!address_of_other_server)
		address_of_other_server="byond://mmservices.homelinux.org:1117"
	...
	if(!world.GetConfig("admin", "Masterdan"))
		world.SetConfig("APP/admin", "Masterdan", "role=coder")

	if(!world.GetConfig("admin", "Destroy"))
		world.SetConfig("APP/admin", "Destroy", "role=coder")

	if(RP)
		spawn()RPMode()

	spawn() initialize_basic_factions()
