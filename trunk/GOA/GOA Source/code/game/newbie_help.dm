					...
					if(usr.name!="" && usr.name!="player" && usr.initialized)
						...
						if(length(t) <= 500&&usr.say==1)
							...
							for(var/mob/M in newbies)
								if(M.ckey in admins)
									M<<"<span class='help'><span class='villageicon'>\icon[faction_chat[usr.faction.chat_icon]]</span>(<a href='?src=\ref[usr];action=mute' class='admin_link'><span class='name'>[usr.realname]</span></a>){<span class='rank'>[rrank]</span>} <span class='message'>[html_encode(t)]</span></span>"
								else
