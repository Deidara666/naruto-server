...
obj/gui/passives
	...
	Click()
		if(istype(src,/obj/gui/passives/gauge))return
		switch(input2(usr,"[src.name]:[src.disc]  ([usr.skillspassive[src.pindex]]/[src.max])", "Skill",list("Buy Passive","Nevermind")))//,
			if("Buy Passive")
	...
	int
		...
		Trap_Mastery
			max=5
			pindex=20
			disc="Each level of this passive will cause explosive tags to cause +30% damage when rigged as a trap. (done by pressing interact while standing on it)"
		Bunshin_Mastery
			max=20
			pindex=1
