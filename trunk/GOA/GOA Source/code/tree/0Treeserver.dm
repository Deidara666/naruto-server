var
	dontdoshit=0
	fuckupcount=0
	PASSWORD="EOA8?rVD7yp-O%`Js:R+_qd1Ms9%P%x4QBl"
	list/Serverlist=new
	save_system/mysql/saves
	sql_host
	sql_database
	sql_user
	sql_password
...
proc
	...
	update_helpers(addr)
		...
		for(var/village in helpers)
			var/topic = list("action" = "update_helpers", "village" = village, "names" = dd_list2text(helpers[village],";"), "Password"=PASSWORD)
			if(!addr)
...
world/New()
	...
	saves = new /save_system/mysql(sql_host, sql_database, sql_user, sql_password)
	if(!saves.ready)
		del world
