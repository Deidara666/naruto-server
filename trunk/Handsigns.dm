...
mob/verb
	PracticeSigns()
		...
		switch(input("What hand signs do you wish to use?",text) in list ("Tiger","Dog","Dragon","Horse","Ram","Ox","Rat","Boar","Monkey","Snake","Bird","Rabbit","EXICUTE"))
			if("Tiger")
				if(usr.Otiger == 1)
					usr<<"Sinal de mãos do Tigre."
					usr.tiger += 1
					return
				else
					usr<<"Você não sabe como fazer esse sinal, talvez, alguém vai lhe mostrar como."
			if("Dog")
				if(usr.Odog == 1)
					usr<<"Sinal de mãos do Cachorro."
					usr.dogs += 1
					return
				else
					usr<<"Você não sabe como fazer esse sinal, talvez, alguém vai lhe mostrar como."
